...
To see if there is a way to group movies with similar attributes together we needed to perform hierarchial clustering using the gower.dist function. Our initial issue was attempting to use Euclidean distance to create the clusterings. This proved diffuclut due to the categorical variables. Using the Gower distance function we were able to work around this and create hierarchial clusterings for a full variable model and a reduced variable model. 
...
In the Full Hierarchial Clustering model, after returning the list of movies in the first cluster, we can see the movie titles included: Iron Man, Spider-Man: Homecoming, Wonder Woman. These movies definitely have simialrities and would definitely be recommended if any movie was selected first and does correlate with the means and proportions we gathered from our function. However, despite the proportions for variables being higher in the Reduced Variable Hierarchial Clustering model, the movie title list returned from the first cluster has much more variability, for example "The Proposal", a romantic comedy, is being recommended in the same category as "The Twilight Saga: Eclipse", a fantasy movie. 
