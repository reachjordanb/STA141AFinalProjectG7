...
We removed variables as an exploration to give us a clustering we believed would be intuitive to compare to the full model. The Full Hierarchial Clustering in figure 5.2 (RIGHT) is the full model with all the variable and the Reduced Variable clustering in figure 5.2 (lEFT) only hasthe variables rating, genre, year, score, and budget. The Reduced Variable Hierarchial Clustering ended up with 6 clades, while the Full Clustering had 3. 
...
```
```{r reducedhclustMeans, fig.cap='Mean and Proportions of Reducaed Hierarchical Clustering', fig.show = 'hold', fig.align = 'center', echo=FALSE,warning=FALSE,out.width='49%',out.height='65%'}
...
In comparing the first cluster from each table, we see that the mean rating and genre are the same, although the proportions are different. The Reduced Variable clustering from figure 5.3 had a higher proportion of rating and lower proportion of genre. The year and scores are slightly different, however, the biggest difference was in the variable budget, which was much higher for the Reduced Variable clustering. 
Looking at the Full clustering table from figure 5.4, we that the mean year, votes, and budget increase between cluster 1 to cluster 3, whereas in the Reduced Variable clustering table, we do not see this same pattern in those variables between clusters 1 to 6. 
