...
```{r groupContributionsEmails, fig.align = 'center', echo=FALSE,warning=FALSE,out.width='50%',out.height='50%'}
...
The gower.dist function computes the Gower's distance (dissimilarity) between units in a dataset or between observations in two distinct datasets. It is useful in cases where records contain combinations of logical, numerical, categorical or text data. 
...
In comparing the first cluster from each table, we see that the mean rating and genre are the same, although the proportions are different. The Reduced Variable clustering from figure \@ref(fig:reducedhclustMeans) has a 0.99 proportion for the rating variable, which means 99% of the movies in that clustering share the same rating of R. Clusters 2 thrugh 6 in the Reduced Variable clustering also have relatively high proportions. Therefore, the movies are being grouped by rating pretty accurately. The genre variable has proportions slighly varied, where some clusters have a lower proportion like 0.32, other clusters have a proportion of 1, which means all the movies in that cluster have the same genre. The year and scores are slightly different, however, the biggest difference was in the variable budget, which was much higher for the Reduced Variable clustering. 
Looking at the Full clustering table from figure \@ref(fig:reducedhclustMeans), we that the mean year, votes, and budget increase between cluster 1 to cluster 3, whereas in the Reduced Variable clustering table, we do not see this same pattern in those variables between clusters 1 to 6. 
