...
For the Reduced Variables Hierarchial Clustering, we removed the variables that were found the be not significant, leaving the reduced model with rating, genre, year, score, adn budget. The most interesting finding in the Full Hierarchial Clustering was the red cluster, which had a different clade than the green and blue. The Reduced Variable Hierarchial Clustering ended up with 6 cclades, while the Full Clustering had 3.

Movies from full hierarchical clustering:
  
-- image

Movies from reduced variable hierarchical clustering:
-- image

We used a function to find the mean and proportion of each variable in each cluster of both models:
```{r dendPlots, fig.cap='Means of variables of full and reduced variable hierarchical clustering', fig.show = 'hold', fig.align = 'center', echo=FALSE,warning=FALSE,out.width='49%',out.height='65%'}
knitr::include_graphics(c("plots/full_hclust_means.png", "plots/reduced_hclust_means.png"))
```
...
In comparing the first clade of each clustering, we see that the mean rating and genre are the same, however the proportion of rating is higher and the proportion for genre is lower in the Reduced clustering. The year and scores are slightly different, however, the budget for the Reduced model is much higher than the budget for the Full clustering. 
