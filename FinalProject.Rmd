...
means_by_cluster <- function(cut_hclust, df){
  ...
  center_df <- as.data.frame(matrix(NA, nrow = (max(unique(cut_hclust)) + 1), ncol = dim(df)[2]))
  colnames(center_df) <- names(df)
...
# make into tables
means_by_cluster(cut_gow_full_hclust, movies_clust)
means_by_cluster(cut_gow_intuitive_hclust, movies_clust[c(1, 3, 5, 7, 8)])
